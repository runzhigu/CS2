...
public class NGramMap {
    ...
    public IPriorityQueue.PQElement<String>[] getCountsAfter(NGram ngram) {
        //get the map of iterable strings to word counts
        IDictionary<IterableString, Integer> nGramInner = map.get(ngram);
        if (nGramInner != null) {
            IPriorityQueue.PQElement<String>[] nGramQueue = new IPriorityQueue.PQElement[nGramInner.size()];
            int index = 0;
            for (IterableString str : nGramInner.keys()) {
                IPriorityQueue.PQElement element = new IPriorityQueue.PQElement(str.toString(), nGramInner.get(str));
                nGramQueue[index] = (element);
                index++;
            }
            return nGramQueue;
        }
    ...
    public String[] getWordsAfter(NGram ngram, int k) {
        IPriorityQueue.PQElement<String>[] nGramQueue = getCountsAfter(ngram);
        if (nGramQueue != null) {
            String[] acc;
            if (nGramQueue.length < k) {
                acc = new String[nGramQueue.length];
            }
            else {
                acc = new String[k];
            }
            TopKSort nGramSorter = new TopKSort();
            nGramSorter.sort(nGramQueue, k);
            for (int i = 0; i < nGramQueue.length; i++) {
                acc[i] = (String) nGramQueue[i].data;
            }
            return acc;
        }
        return new String[0];
